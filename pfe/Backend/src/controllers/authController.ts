...
export const SignupClient = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!username || !email || !password) {
    res.status(400).json({ message: "Nom d'utilisateur, email et mot de passe requis" });
    return;
  }

  try {
    const user = await UserService.createUser(username, email, password, UserRole.CLIENT);
...
export const SignupSecretaire = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const user = await UserService.createUser(username, email, password, UserRole.SECRETAIRE);
...
export const LoginClient = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const token = await UserService.authenticateUser(username || email, password); // Utilisation de email ou username
